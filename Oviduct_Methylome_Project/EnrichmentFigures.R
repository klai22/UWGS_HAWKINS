...
#import data 
od12 = read.csv("od12enrichment_all.csv")
od13 = read.csv("od13enrichment_all.csv")
od1o = read.csv("od1oenrichment_all.csv")
od23 = read.csv("od23enrichment_all.csv")
od2o = read.csv("od2oenrichment_all.csv")
od3o = read.csv("od3oenrichment_all.csv")
...
FDRheatmap=ggplot(masterFDR3)+ geom_tile(aes(x=tissuescompared,y=Pathway,fill=FDR.Enrichment))+  scale_colour_gradient2(
  ...
  theme(axis.text.x=element_text(angle=20,size=9, vjust=0.88,hjust=0.9),
        ...
        plot.margin=margin(0.8,0.8,0.8,0.8,"cm"),legend.position="right")+ ggtitle("DMR KEGG Pathways (FDR Enrichment-Based)")

#printing the graph 
pdf("FDR.pdf", width=10,height=8)

print(FDRheatmap,
      labels=c("DMR KEGG Pathways (FDR Enrichment-Based)"),
...
nDMRsheatmap=ggplot(masterngenes3)+ geom_tile(aes(x=tissuescompared,y=Pathway,fill=`DMRs Detected`))+ scale_colour_gradient2(
  ...
  theme(axis.text.x=element_text(angle=30,size=9, vjust=0.88,hjust=0.9),
        ...
        axis.title.x=element_text(size=11),
        axis.title.y = element_text(size=11),
        plot.margin=margin(0.8,0.8,0.8,0.8,"cm"),legend.position="right")+ ggtitle("DMR KEGG Pathways (nGenes-based)")


#printing the graph 
pdf("nDMR.pdf", width=10,height=8)

print(nDMRsheatmap,
      labels=c("DMR KEGG Pathways (nGenes-based)"),
      ncol=1,nrow=1,legend = NULL,
      common.legend = TRUE,
      legend.grob = NULL)
