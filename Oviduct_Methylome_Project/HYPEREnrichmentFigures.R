...
#import data 
od12 = read.csv("hyperod12enrichment.csv")
od13 = read.csv("hyperod13enrichment.csv")
od1o = read.csv("hyperod1oenrichment.csv")
od23 = read.csv("hyperod23enrichment.csv")
od2o = read.csv("hyperod2oenrichment.csv")
od3o = read.csv("hyperod3oenrichment.csv")
...
FDRheatmap=ggplot(masterFDR3)+ geom_tile(aes(x=tissuescompared,y=Pathway,fill=FDR.Enrichment))+  scale_colour_gradient2(
  ...
  theme(axis.text.x=element_text(angle=20,size=9, vjust=0.88,hjust=0.9),
        ...
        plot.margin=margin(0.8,0.8,0.8,0.8,"cm"),legend.position="right")+ ggtitle("Hypermethylated DMR KEGG Pathways (FDR Enrichment-Based)")

#printing the graph 
pdf("HyperFDR.pdf", width=10,height=8)

print(FDRheatmap,
      labels=c("Hypermethylated DMR KEGG Pathways (FDR Enrichment-Based)"),
...
nDMRsheatmap=ggplot(masterngenes3)+ geom_tile(aes(x=tissuescompared,y=Pathway,fill=`DMRs Detected`))+ scale_colour_gradient2(
  ...
  theme(axis.text.x=element_text(angle=30,size=9, vjust=0.88,hjust=0.9),
        ...
        axis.title.x=element_text(size=11),
        axis.title.y = element_text(size=11),
        plot.margin=margin(0.8,0.8,0.8,0.8,"cm"),legend.position="right")+ ggtitle("Hypermethylated DMR KEGG Pathways (nGenes-based)")


#printing the graph 
pdf("HypernDMR.pdf", width=10,height=8)

print(nDMRsheatmap,
      labels=c("Hypermethylated DMR KEGG Pathways (nGenes-based)"),
      ncol=1,nrow=1,legend = NULL,
      common.legend = TRUE,
      legend.grob = NULL)
