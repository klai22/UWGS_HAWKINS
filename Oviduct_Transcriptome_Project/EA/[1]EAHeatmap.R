#Kenneth Lai
# setwd 
setwd("/Users/kennethlai/desktop/HAWKINS/FAANg/RNAseq/EA/2")
...
#import data 
od12 = read.csv("[od12]enrichment_all.csv")
od13 = read.csv("[od13]enrichment_all.csv")
od1o = read.csv("[od1o]enrichment_all.csv")
od23 = read.csv("[od23]enrichment_all.csv")
od2o = read.csv("[od2o]enrichment_all.csv")
od3o = read.csv("[od3o]enrichment_all.csv")
...
#moving fold enrichment data around to make 2 columns out of it so that it can be used in heatmap 
...
#finding the median value of FDRenrichment so I can apply it to the color correctly 
median(masterFDR3$Fold.Enrichment, na.rm = TRUE)
# the median = 1.671219

#finding the average as a replacement for the median value 
mean(range(masterFDR3$Fold.Enrichment,na.rm = TRUE))
# average = 2.558882

#ggplot for the heatmap 
FDRheatmap=ggplot(masterFDR3)+ geom_tile(aes(x=tissuescompared,y=Pathway,fill=Fold.Enrichment))+  scale_colour_gradient2(
  low ="lightblue",
  mid = "black",
  high = "pink",
  midpoint = 2.558882,
  ...
  guide = "colourbar",
  aesthetics = "fill")+
  labs(x="Tissues Compared", y="KEGG Pathway",fill="Fold Enrichment Value") + 
  ...
  theme(axis.text.x=element_text(angle=20,size=9, vjust=0.88,hjust=0.9),
        ...
        plot.margin=margin(0.8,0.8,0.8,0.8,"cm"),legend.position="right")+ ggtitle("RNAseq KEGG Pathways (Fold Enrichment-Based)")

#printing the graph 
pdf("EAHeatmap[FE].pdf", width=10,height=8)

print(FDRheatmap,
      labels=c("RNAseq KEGG Pathways (Fold Enrichment-Based)"),
...
median(masterngenes3$`Transcripts Detected`)
# the median = 0 

#finding the average as a replacement for the median value 
mean(range(masterngenes3$`Transcripts Detected`))
# average = 218
...
#ggplot for the heatmap 
nGsheatmap=ggplot(masterngenes3)+ geom_tile(aes(x=tissuescompared,y=Pathway,fill=`Transcripts Detected`))+ scale_colour_gradient2(
  low ="lightblue",
  mid = "black",
  high = "pink",
  midpoint = 218,
  ...
  guide = "colourbar",
  aesthetics = "fill")+
  labs(x="tissuescompared", y="KEGG Pathway",fill="Transcripts Detected") + 
  ...
  theme(axis.text.x=element_text(angle=30,size=9, vjust=0.88,hjust=0.9),
        ...
        axis.title.x=element_text(size=11),
        axis.title.y = element_text(size=11),
        plot.margin=margin(0.8,0.8,0.8,0.8,"cm"),legend.position="right")+ ggtitle("RNAseq KEGG Pathways (nGenes-based)")


#printing the graph 
pdf("EAHeatmap[nGenes].pdf", width=10,height=8)

print(nGsheatmap,
      labels=c("RNAseq KEGG Pathways (nGenes-based)"),
      ncol=1,nrow=1,legend = NULL,
      common.legend = TRUE,
      legend.grob = NULL)
